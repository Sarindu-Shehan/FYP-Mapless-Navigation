...
class Actor():
    def __init__(self, filename='./saved/Actor.h5'):
        ...
        w = tf.initializers.random_uniform(-0.001, 0.001)
        self.input = tf.keras.layers.Input(shape=[16])

        x = tf.keras.layers.Dense(256)(self.input)
        ...
        out = tf.keras.layers.Lambda(lambda x: x * 1)(out)
...
class Critic():
    def __init__(self, filename='./saved/Critic.h5'):
        ...
        x = tf.keras.layers.Dense(300)(self.input)
        # x = tf.keras.layers.Dense(400)(self.input)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation(tf.nn.relu)(x)
        x = tf.keras.layers.Dense(300)(x)
    ...
    def train_step(self, state, action, predicted_q_value):
        ...
        with tf.GradientTape() as tape:
            ...
            # print("predicted qs:",predicted_q_value)

            loss = tf.compat.v1.losses.mean_squared_error(predictions, predicted_q_value)
